from math import * #*-kõik funktsioonid on vaja kasutada
...
Skr=pi*r**2
print(f"Ristküliku pindala on {S}. Ja ringi pindala on {round(Skr,2)}.")
...
print(f"{nimi} Jargmisel aastal saad {vanus+1}") #muutja vanus ei muuda
print("Aga selle aastal on ", vanus)
vanus+=1 #muudame muutja väärtus
print(f"Jargmine aasta on käes, siis {nimi} on {vanus} aastat vana")
pikkus=float(input("Mis on sinu pikkus?")) #input()->float: 1.75
print(f"{nimi} on {vanus} aastat vana ja suurpärase pikkusega {pikkus}")